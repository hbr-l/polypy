...
class OrderBookStream:
    ...
    def _process_raw_message(self, raw_messages: str | bytes) -> None:
        ...
        for msg in raw_messages:
            if self.lock_dict[msg["asset_id"]].locked():
                # todo necessary?
                # discard if orderbook is being processed
                continue
    ...
    def _update_book(
        ...
        if not self._check_hash(hash_status, msg) and self.endpoint is not None:
            warnings.warn(
                f"{datetime.datetime.now()}: REST fetch orderbook due to invalid orderbook hash."
            )

            resp = get_orderbook(self.endpoint, msg["asset_id"])
    ...
    def _guess_hash(self, asset_id: str, book_id: int, msg: dict[str, Any]) -> bool:
        ...
        warnings.warn(
            f"{datetime.datetime.now()}: Invalid orderbook hash. Message: {msg}."
        )
        self.status_arr[book_id] = -1
        return False

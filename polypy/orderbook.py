...
if TYPE_CHECKING:
    from polypy.order.base import SIDE
...
class OrderBook:
    ...
    def _prices_to_indices(self, price: list[NumericAlias] | np.ndarray) -> np.ndarray:
        idx = (np.array(price) * self._inv_min_tick_size).astype(int)
    ...
    def marketable_price(
        self, side: str | Literal["BUY", "SELL"] | "SIDE", amount: NumericAlias
...
# todo use Enum for event_type? (also in OrderBookStream._update_last_traded_price)
...
def calculate_marketable_price(
    ...
    amount_levels = np.array(price_levels) * np.array(size_levels)

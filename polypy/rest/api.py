...
def _get_book_summary(endpoint: str | ENDPOINT, asset_id: str) -> BookSummary:
    resp = _request(f"{endpoint}/book?token_id={asset_id}", "GET", None, None)
    return msgspec.json.decode(resp.text, type=BookSummary, strict=False)


def get_book_summaries(
    endpoint: str | ENDPOINT, asset_ids: str | list[str]
) -> BookSummary | dict[str, BookSummary]:
    ...
    resp = msgspec.json.decode(resp.text, type=list[BookSummary], strict=False)

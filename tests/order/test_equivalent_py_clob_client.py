...
from polypy.order import SIDE, SIGNATURE_TYPE, build_limit_order, build_market_order
...
def test_limit_order_amount(
    ...
    while size <= max_size:
        price = price_init
        while price <= 1:
            maker, taker, _, _ = limit_order_taker_maker_amount(
...
def test_market_order_amount(
    ...
    while amount <= max_amount:
        price = price_init
        while price <= 1:
            maker, taker, _, _ = market_order_taker_maker_amount(
...
def test_create_limit_order_decimal_accuracy(
    ...
    buy_order = build_limit_order(
        0.24, 15, "123", SIDE.BUY, 0.01, private_key, CHAIN_ID.POLYGON, neg_risk
    )
    buy_order_dict = buy_order.to_dict()
    assert buy_order_dict["makerAmount"] == "3600000"
    assert buy_order_dict["takerAmount"] == "15000000"

    sell_order = build_limit_order(
        0.24, 15, "123", SIDE.SELL, 0.01, private_key, CHAIN_ID.POLYGON, neg_risk
    )
    ...
    buy_order = build_limit_order(
        0.82, 101, "123", SIDE.BUY, 0.01, private_key, CHAIN_ID.POLYGON, neg_risk
    )
    buy_order_dict = buy_order.to_dict()
    ...
    sell_order = build_limit_order(
        0.82, 101, "123", SIDE.SELL, 0.01, private_key, CHAIN_ID.POLYGON, neg_risk
    )
    sell_order_dict = sell_order.to_dict()
    ...
    buy_order = build_limit_order(
        0.78, 12.8205, "123", SIDE.BUY, 0.01, private_key, CHAIN_ID.POLYGON, neg_risk
    )
    buy_order_dict = buy_order.to_dict()
    ...
    sell_order = build_limit_order(
        0.78, 12.8205, "123", SIDE.SELL, 0.01, private_key, CHAIN_ID.POLYGON, neg_risk
    )
    sell_order_dict = sell_order.to_dict()
    ...
    sell_order = build_limit_order(
        0.39, 2435.89, "123", SIDE.SELL, 0.01, private_key, CHAIN_ID.POLYGON, neg_risk
    )
    sell_order_dict = sell_order.to_dict()
    ...
    sell_order = build_limit_order(
        0.43, 19.1, "123", SIDE.SELL, 0.01, private_key, CHAIN_ID.POLYGON, neg_risk
    )
    sell_order_dict = sell_order.to_dict()
    ...
    buy_order = build_limit_order(
        0.58, 18233.33, "123", SIDE.BUY, 0.01, private_key, CHAIN_ID.POLYGON, neg_risk
    )
    buy_order_dict = buy_order.to_dict()

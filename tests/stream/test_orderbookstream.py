...
def assert_orderbook_streamer_state(
    streamer: OrderBookStream,
    book: OrderBook,
    market_id: str,
...
def test_orderbookstream_delay_partial_drop_skip_no_request(
    ...
    with warnings.catch_warnings():
        ...
        assert_orderbook_streamer_state(
            streamer,
            book,
            market_id,
            1,
    ...
    assert "Invalid orderbook hash" in str(record[0].message)
    ...
    assert_orderbook_streamer_state(*assert_args)
...
@responses.activate
def test_orderbookstream_fallback_mock_request(mock_server_click_on, setup_streamer):
    click_on, asset_id, data = mock_server_click_on(
        data_pth=test_pth / "data/test_server_messages.txt"
    )
    streamer, book = setup_streamer(
    ...
    with pytest.warns() as record:
        click_on.send(2)
        # this should force a REST GET to manually fetch orderbook

    assert len(record) == 2  # emits warning
    assert "Invalid orderbook hash" in str(record[0].message)
    assert "REST fetch" in str(record[1].message)
    # after manually fetching, orderbook should be OK again
    assert_orderbook_streamer_state(
        streamer,
        book,
        market_id,
...
def test_orderbookstream_wrong_asset_id_and_callback_exception(
...
def test_orderbookstream_unknown_event_type_and_callback_exception(
    mock_server_click_on, setup_streamer, callback_exception_storage
):
    click_on, asset_id, data = mock_server_click_on(
...
def test_orderbookstream_rest_denial_and_callback_exception(
    ...
    with pytest.warns() as record:
        click_on.send(2)
        # this should force a REST GET to manually fetch orderbook -> denial
...
@responses.activate
def test_orderbookstream_rest_denial_no_recursion(
    mock_server_click_on, setup_streamer, callback_exception_storage
):
    """
    Test if we run into recursion if hash in book message is wrong (which never will be the case).
    Note: despite orderbook being corrupted, we do not expect any exception, because we do not
    ...
    with pytest.warns() as record:
        click_on.send(2)
        # this should force a REST GET to manually fetch orderbook -> wrong hash

    assert len(record) == 2  # emitted warning
    assert "Invalid orderbook hash" in str(record[0].message)
    assert "REST fetch" in str(record[1].message)

    # despite orderbook being corrupted, we do not expect any exceptions because
...
def test_orderbookstream_callback_message(
...
# noinspection DuplicatedCode
def test_orderbookstream_multi_last_trade_price(mock_server_click_on, setup_streamer):
    click_on, asset_id, data = mock_server_click_on(
...
# noinspection DuplicatedCode
def test_orderbookstream_multi_book(mock_server_click_on):
...
def test_orderbookstream_reconnect(
    mock_server_click_on, setup_streamer, callback_thread_id_storage
...
def test_orderbookstream_nb_sockets_and_buffer_protection(
    mock_server_click_on, setup_streamer, callback_thread_id_storage
):
    click_on, asset_id, data = mock_server_click_on(
        data_pth=test_pth / "data/test_server_messages.txt"
